...
describe("helpers/getter/getSiteNoneCompatible", () => {
  for (const i in positiveTestCases) {
    if (positiveTestCases.hasOwnProperty(i)) {
      it(`Test ${i} (true)`, () => {
        expect(getSiteNoneCompatible(positiveTestCases[i])).toBe(true)
      })
    }
  }
  ...
  for (const i in negativeTestCases) {
    if (negativeTestCases.hasOwnProperty(i)) {
      it(`Test ${i} (false)`, () => {
        expect(getSiteNoneCompatible(negativeTestCases[i])).toBe(false)
      })
    }
  }
})
